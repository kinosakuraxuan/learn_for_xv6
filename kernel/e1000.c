...
e1000_recv(void)
{
  for(;;){
    ...
    struct mbuf *received = rx_mbufs[index];
    received->len = desc->length;
    ...
    // net_rx() may send a reply and re-enter e1000_transmit(), so it
    // must run without holding e1000_lock.
    net_rx(received);
  }
}
